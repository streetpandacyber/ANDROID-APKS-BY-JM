...
export type NotebookSelection = { start: number; end: number };
export function toggleNotebookFormatting(body: string, token: string, selection: NotebookSelection = { start: body.length, end: body.length }) { const start = Math.max(0, Math.min(selection.start, body.length)); const end = Math.max(start, Math.min(selection.end, body.length)); const wrappers: Record<string, { open: string; close: string; placeholder: string }> = { B: { open: "**", close: "**", placeholder: "bold" }, I: { open: "*", close: "*", placeholder: "italic" }, U: { open: "<u>", close: "</u>", placeholder: "underline" }, S: { open: "~~", close: "~~", placeholder: "strikethrough" } }; const wrapper = wrappers[token]; if (wrapper) { if (end > start) { const selected = body.slice(start, end); if (selected.startsWith(wrapper.open) && selected.endsWith(wrapper.close)) { const unwrapped = selected.slice(wrapper.open.length, selected.length - wrapper.close.length); return { text: `${body.slice(0, start)}${unwrapped}${body.slice(end)}`, selection: { start, end: start + unwrapped.length }, toggledOff: true }; } if (body.slice(start - wrapper.open.length, start) === wrapper.open && body.slice(end, end + wrapper.close.length) === wrapper.close) { const unwrapped = body.slice(start, end); const newStart = start - wrapper.open.length; return { text: `${body.slice(0, newStart)}${unwrapped}${body.slice(end + wrapper.close.length)}`, selection: { start: newStart, end: newStart + unwrapped.length }, toggledOff: true }; } const wrapped = `${wrapper.open}${selected}${wrapper.close}`; return { text: `${body.slice(0, start)}${wrapped}${body.slice(end)}`, selection: { start: start + wrapper.open.length, end: start + wrapper.open.length + selected.length }, toggledOff: false }; } const placeholder = `${wrapper.open}${wrapper.placeholder}${wrapper.close}`; const placeholderStart = start - placeholder.length; if (body.slice(placeholderStart, start) === placeholder) { const removeStart = placeholderStart > 0 && body[placeholderStart - 1] === " " ? placeholderStart - 1 : placeholderStart; return { text: `${body.slice(0, removeStart)}${body.slice(start)}`, selection: { start: removeStart, end: removeStart }, toggledOff: true }; } return { ...formatNotebookInsertion(body, token, { start, end }), toggledOff: false }; } const lineStart = body.lastIndexOf("\n", Math.max(0, start - 1)) + 1; const lineEndIndex = body.indexOf("\n", end); const lineEnd = lineEndIndex === -1 ? body.length : lineEndIndex; const line = body.slice(lineStart, lineEnd); const prefixes: Record<string, string> = { H1: "# ", H2: "## ", H3: "### ", "• List": "• ", "1. List": "1. ", "☐ Task": "☐ ", "—": "---" }; const prefix = prefixes[token]; if (!prefix) return { ...formatNotebookInsertion(body, token, { start, end }), toggledOff: false }; const currentPrefix = line.match(/^(#{1,3}\s|(?:[-*•]\s|\d+\.\s)|☐\s|---+)/)?.[0] || ""; if (currentPrefix === prefix) return { text: `${body.slice(0, lineStart)}${line.slice(prefix.length)}${body.slice(lineEnd)}`, selection: { start: Math.max(lineStart, start - prefix.length), end: Math.max(lineStart, end - prefix.length) }, toggledOff: true }; const replacement = `${prefix}${currentPrefix ? line.slice(currentPrefix.length) : line}`; return { text: `${body.slice(0, lineStart)}${replacement}${body.slice(lineEnd)}`, selection: { start: start + prefix.length - currentPrefix.length, end: end + prefix.length - currentPrefix.length }, toggledOff: false }; }
export function formatNotebookInsertion(body: string, token: string, selection: NotebookSelection = { start: body.length, end: body.length }) { const start = Math.max(0, Math.min(selection.start, body.length)); const end = Math.max(start, Math.min(selection.end, body.length)); const templates: Record<string, string> = { B: "**bold**", I: "*italic*", U: "<u>underline</u>", S: "~~strikethrough~~", H1: "# ", H2: "## ", H3: "### ", "• List": "• ", "1. List": "1. ", "☐ Task": "☐ ", "—": "---", "↔": "  " }; const insertion = templates[token] || ""; const needsLeadingSpace = start > 0 && !/\s/.test(body[start - 1]) && !/^\s/.test(insertion); const prefix = needsLeadingSpace ? " " : ""; const text = `${body.slice(0, start)}${prefix}${insertion}${body.slice(end)}`; const cursor = start + prefix.length + insertion.length; return { text, selection: { start: cursor, end: cursor } }; }
export type NotebookEditSnapshot = { body: string; selection: NotebookSelection };
export type NotebookHistoryDirection = "undo" | "redo";
export function moveNotebookHistory(current: NotebookEditSnapshot, undo: NotebookEditSnapshot[], redo: NotebookEditSnapshot[], direction: NotebookHistoryDirection, maxEntries = 50) { const source = direction === "undo" ? undo : redo; if (!source.length) return { current, undo, redo, moved: false }; const next = source[source.length - 1]; const boundedCurrent = { ...current }; const boundedUndo = direction === "undo" ? source.slice(0, -1) : [...undo.slice(-(maxEntries - 1)), boundedCurrent]; const boundedRedo = direction === "undo" ? [...redo.slice(-(maxEntries - 1)), boundedCurrent] : source.slice(0, -1); return { current: next, undo: boundedUndo, redo: boundedRedo, moved: true }; }
