...
describe("offline business rules", () => {
  ...
  it("toggles Notebook formatting off when the same control is used again", () => {
    const bold = toggleNotebookFormatting("hello", "B", { start: 0, end: 5 });
    expect(bold.text).toBe("**hello**");
    expect(bold.selection).toEqual({ start: 2, end: 7 });
    const unbold = toggleNotebookFormatting(bold.text, "B", bold.selection);
    expect(unbold.text).toBe("hello");
    expect(unbold.toggledOff).toBe(true);
    const heading = toggleNotebookFormatting("item", "H2", { start: 0, end: 4 });
    expect(heading.text).toBe("## item");
    expect(toggleNotebookFormatting(heading.text, "H2", heading.selection).text).toBe("item");
    expect(toggleNotebookFormatting("• item", "• List", { start: 2, end: 6 }).text).toBe("item");
  });

  it("inserts the requested Notebook formatting at the cursor or selection", () => {
    expect(formatNotebookInsertion("hello", "B")).toMatchObject({ text: "hello **bold**", selection: { start: 14, end: 14 } });
    expect(formatNotebookInsertion("hello world", "I", { start: 6, end: 11 }).text).toBe("hello *italic*");
    expect(formatNotebookInsertion("", "H1").text).toBe("# ");
    expect(formatNotebookInsertion("", "H2").text).toBe("## ");
    expect(formatNotebookInsertion("", "H3").text).toBe("### ");
    expect(formatNotebookInsertion("", "• List").text).toBe("• ");
  });
